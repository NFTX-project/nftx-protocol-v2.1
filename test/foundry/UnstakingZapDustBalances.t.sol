...
contract UnstakingZapDustBalances is Test {
    ...
    address internal constant PUNK_HOLDER =
        0xA858DDc0445d8131daC4d1DE01f834ffcbA52Ef1;
    address internal constant PUNK_TOKEN =
        0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB;

    // Our PUNK vault
    uint256 internal constant VAULT_ID = 0;
    address internal constant VAULT_ADDR =
        0x269616D549D7e8Eaa82DFb17028d0B212D11232A;
    address internal constant VAULT_XTOKEN =
        0x08765C76C758Da951DC73D3a8863B34752Dd76FB;

    // Our zaps
    address payable internal constant STAKING_ZAP =
        payable(0xdC774D5260ec66e5DD4627E1DD800Eff3911345C);
    address payable internal constant UNSTAKING_ZAP =
        payable(0x51d660Ba5c218b2Cf33FBAcA5e3aBb8aEff3543B);
    ...
    function test_noUnderflow(uint256 amount) public {
        ...
        uint256[] memory selectedTokenIds = new uint256[](amount);
        for (uint256 i; i < amount; ) {
            ...
            (bool success, bytes memory resultData) = address(PUNK_TOKEN).call(
                data
            );
        ...
        NFTXStakingZap(STAKING_ZAP).provideInventory721(
            VAULT_ID,
            selectedTokenIds
        );

        // Get our new xToken balance
        uint256 xTokenBalance = IERC20(VAULT_XTOKEN).balanceOf(PUNK_HOLDER);
        ...
        (uint256 numNfts, bool shortByTinyAmount) = NFTXUnstakingInventoryZap(
            UNSTAKING_ZAP
        ).maxNftsUsingXToken(VAULT_ID, PUNK_HOLDER, VAULT_XTOKEN);
        ...
        uint256 iniPunkBalance = IERC721Upgradeable(PUNK_TOKEN).balanceOf(
            PUNK_HOLDER
        );
        ...
        assertEq(
            IERC721Upgradeable(PUNK_TOKEN).balanceOf(PUNK_HOLDER) -
                iniPunkBalance,
            amount
        );
    ...
    function determineBurnXTokenValue(uint256 _share) internal {
        /**
        ...
        uint256 xTokenBaseBalance = IERC20(VAULT_ADDR).balanceOf(
            address(VAULT_XTOKEN)
        );
